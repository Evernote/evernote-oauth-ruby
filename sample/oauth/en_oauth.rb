...
get '/requesttoken' do
  ...
  begin
    client = EvernoteOAuth::Client.new(consumer_key: OAUTH_CONSUMER_KEY,
...
##
get '/list' do
  note_store = EvernoteOAuth::Client.new(token: session[:access_token].token,
					 sandbox: SANDBOX).note_store

  begin
    # All NoteStore API calls require you to pass the OAuth token credentials
    # (aka access token) as the auth token
    notebooks = note_store.listNotebooks(session[:access_token].token)
...
<html>
  ...
  <body>
    ...
    <p>
      <b>Evernote server:</b> (sandbox: <%= SANDBOX %>)<br />
